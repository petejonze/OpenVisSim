...
namespace VisSim
{
	public class myFieldLoss : LinkableBaseEffect
    {
        ...
        protected void Awake() // (NB: not Start, as Grid needs to be set even if not enabled)
        {
            ...
            // glaucoma
            double[,] xy = new double[,] { { -21, 9, -20 }, { -21, 3, -20 }, { -21, -3, -20 }, { -21, -9, -20 }, { -15, 15, -15 }, { -15, 9, -15 }, { -15, 3, -20 }, { -15, -3, -15 }, { -15, -9, -15 }, { -15, -15, -10 }, { -9, 15, -20 }, { -9, 9, -10 }, { -9, 3, 0 }, { -9, -3, 0 }, { -9, -9, 0 }, { -9, -15, -10 }, { -3, 15, -20 }, { -3, 9, -8 }, { -3, 3, 0 }, { -3, -3, 0 }, { -3, -9, 0 }, { -3, -15, -20 }, { 3, 15, 0 }, { 3, 9, 0 }, { 3, 3, 0 }, { 3, -3, 0 }, { 3, -9, -8 }, { 3, -15, -20 }, { 9, 15, -20 }, { 9, 9, -10 }, { 9, 3, 0 }, { 9, -3, -5 }, { 9, -9, -10 }, { 9, -15, -20 }, { 15, 15, -20 }, { 15, 9, -8 }, { 15, -9, -15 }, { 15, -15, -20 }, { 21, 9, -20 }, { 21, 3, -20 }, { 21, -3, -20 }, { 21, -9, -15 }, { 27, 3, -20 }, { 27, -3, -20 } };
            this.setGrid(xy);

            /* FOR SCREENSHOTS FOR IEEE PAPER
            byte[] bytes = overlayTexture.EncodeToPNG();
            string filename = string.Format("{0}/screenshots/overlayTexture_{1}.png",
                             Application.dataPath,
                             System.DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss"));
            System.IO.File.WriteAllBytes(filename, bytes);
            Debug.Log(string.Format("Saved fieldmap to: {0}", filename));


            Texture2D overlayTexture_hiRes = Instantiate(overlayTexture);
            TextureScale.Bilinear(overlayTexture_hiRes, 1600, 1600);
            bytes = overlayTexture_hiRes.EncodeToPNG();
            filename = string.Format("{0}/screenshots/overlayTexture_hiRes_{1}.png",
                             Application.dataPath,
                             System.DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss"));
            System.IO.File.WriteAllBytes(filename, bytes);
            Debug.Log(string.Format("Saved fieldmap to: {0}", filename));
        ...
        private void OnGUI()
        {
            GUI.DrawTexture(new Rect(0, 0, Screen.width/2, Screen.height), overlayTexture, ScaleMode.ScaleToFit);
        }
        */
        ...
        public void setGrid(double[,] grid_xy)
        {
            ...
            overlayTexture = GridInterpolator.Instance.interpolateGridAndMakeTexture(grid_xy);
			Material.SetTexture("_Overlay", overlayTexture);
        }
