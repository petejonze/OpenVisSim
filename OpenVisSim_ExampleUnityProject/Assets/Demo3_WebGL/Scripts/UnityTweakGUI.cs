...
public class UnityTweakGUI : MonoBehaviour
{
    ...
    private GUILayoutOption[] wideLabelLayoutOptions = { GUILayout.MaxWidth(120.0f), GUILayout.MinWidth(10.0f) }; // PJ: ensure lefthand labels don't overflow
    ...
    private GUILayoutOption[] wideEnumLayoutOptions = { GUILayout.MaxWidth(350.0f), GUILayout.MinWidth(100.0f) };
    ...
    // visField additional copy-paste params
    private bool showVisFieldCopyPasteGui = false;
    //private GUILayoutOption[] textAreaLayoutOptions = { GUILayout.MaxWidth(500f), GUILayout.MinWidth(300f), GUILayout.MaxHeight(500f), GUILayout.MinHeight(300f) };
    public Canvas visFieldInputCanvas;
    ...
    // bgrd image
    private int bgrdImageIdx = 0;
    private string[] bgrdImageFn = { "webgl_IMG_0493", "webgl_children_raw_1024", "webgl_15568622684_3232acd542_b", "webgl_6", "stream_image_from_preset_url",  "stream_video_from_preset_url" };
    private string[] bgrdLabel = { "Our Lab", "Children", "Street", "Bus", "IMG", "VID" };
    public GameObject bgrdImageCube;
    public WebglTakeScreenshot webglScreenshot;
    public UnityEngine.Video.VideoPlayer videoPlayer;
    private bool showStreamImageExtraGui = false;
    private string imgURL = "https://appliedpsychophysics.com/images/profile/PJ2.jpg";
    private bool showStreamVideoExtraGui = false;
    private string videoURL = "https://appliedpsychophysics.com/vr/webgl_video.mp4";
    private Rect url_Rect = new Rect(30, 25, 475, 150);
    ...
    void Start()
    {
        ...
        fieldLoss = maincamera.GetComponent<myFieldLoss>();
    ...
    void OnGUI()
    {
        ...
        windowRect = GUILayout.Window(0, windowRect, delegate (int id)
        {
            ...
            if (expanded)
            {
                ...
                // PJ -- add background selector button
                GUILayout.BeginVertical(); // move up to place label on the left
                GUILayout.Label("Background Image:", labelStyle, wideLabelLayoutOptions);
                int newBgrdIdx = GUILayout.SelectionGrid(bgrdImageIdx, bgrdLabel, bgrdImageFn.Length, enumStyle, wideEnumLayoutOptions);
                GUILayout.EndVertical(); // move up to place label on the left


                // PJ -- add screenshot button
                GUILayout.Space(18);
                GUILayout.BeginVertical(); // move up to place label on the left
                GUILayout.Label("Screen Grab:", labelStyle, wideLabelLayoutOptions);
                if (GUILayout.Button("Take Screenshot (or press SPACE)"))
                {
                    //Debug.Log("Booo");
                    webglScreenshot.Screenshot();
                }
                GUILayout.EndVertical(); // move up to place label on the left
                ...
                foreach (var group in sortedGroups)
                {
                    ...
                    if (groupParamsMap.TryGetValue(group, out tweakableParams))
                    {
                        ...
                        if (imageEffect.GetType() == typeof(myFieldLoss))
                        {
                            ...
                            if (GUILayout.Button("Copy-Paste Field Map"))
                            {
                                expanded = false;
                                showVisFieldCopyPasteGui = true;
                                visFieldInputCanvas.gameObject.SetActive(true);

                                // set current grid values as placeholder text
                                double[,] xy = fieldLoss.getGrid();
                                string txt = "";
                                ...
                                txt = txt.Remove(txt.Length - 2); // remove trailing comma
                                visFieldInputCanvas.GetComponentInChildren<UnityEngine.UI.InputField>().text = txt;
                            }
                ...
                if (newBgrdIdx != bgrdImageIdx)
                {
                    //Debug.Log(">>> " + bgrdImageFn[newBgrdIdx] + "  ["+ newBgrdIdx+"]");
                    //Debug.Log(bgrdImageCube.GetComponent<Renderer>().material.mainTexture);
                    bgrdImageIdx = newBgrdIdx;
                    string fn = bgrdImageFn[bgrdImageIdx];
                    Debug.Log(">>> " + fn);
                    if (fn.Equals("stream_video_from_preset_url"))
                    {
                        bgrdImageIdx = -1;
                        showStreamVideoExtraGui = true;
                    }
                    else if (fn.Equals("stream_image_from_preset_url"))
                    {
                        bgrdImageIdx = -1;
                        showStreamImageExtraGui = true;
                    }
                    else
                    {
                        videoPlayer.enabled = false;
                        bgrdImageCube.GetComponent<Renderer>().material.mainTexture = Resources.Load(bgrdImageFn[bgrdImageIdx]) as Texture2D;
                    }
                }
        ...
        }, "OpenVisSim (June 2022)");


        if (showStreamImageExtraGui)
        {
            streamImageExtraGui();
        }

        if (showStreamVideoExtraGui)
        {
            streamVideoExtraGui();
        }
        ...
        if (showVisFieldCopyPasteGui)
        {
            visFieldCopyPasteGui();
        }
    ...
    private void streamImageExtraGui()
    {
        GUILayout.Window(1, url_Rect, delegate (int id1)
        {
            // UI
            GUILayout.BeginVertical();

            // header
            String labelString = "Enter image URL:";
            GUILayout.Label(labelString, groupTextStyle, labelLayoutOptions);
            GUILayout.Space(50); // hack!

            GUILayout.BeginHorizontal();
            imgURL = GUI.TextField(new Rect(30, 50, 400, 40), imgURL);
            GUILayout.EndHorizontal();
            ...
            // buttons
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Cancel", visFieldButtonOptions))
            {
                //expanded = true;
                showStreamImageExtraGui = false;
            }
            if (GUILayout.Button("Get Image From URL", visFieldButtonOptions))
            {
                videoPlayer.enabled = false;

                // stream
                Debug.Log("streaming image: " + imgURL);
                StartCoroutine(streamImage(imgURL));
    ...
    System.Collections.IEnumerator streamImage(string url)
    {
        ...
        bgrdImageCube.GetComponent<Renderer>().material.mainTexture = tex;
    }

    private void streamVideoExtraGui()
    {
        GUILayout.Window(1, url_Rect, delegate (int id1)
        {
            // UI
            ...
            // header
            String labelString = "Enter video URL:";
            GUILayout.Label(labelString, groupTextStyle, labelLayoutOptions);
            GUILayout.Space(50); // hack!

            GUILayout.BeginHorizontal();
            videoURL = GUI.TextField(new Rect(30, 50, 400, 40), videoURL);
            GUILayout.EndHorizontal();
            ...
            {
                //expanded = true;
                showStreamVideoExtraGui = false;
            }
            if (GUILayout.Button("Get Video From URL", visFieldButtonOptions))
            {
                videoPlayer.enabled = false;

                // stream
                Debug.Log("streaming video: " + videoURL);
    ...
    private void visFieldExtraGui()
    {
        ...
        double[,] xy = fieldLoss.getGrid();
            ...
            */
            ...
            if (changed)
            {
                fieldLoss.setGrid(xy, false, false); // don't interpolate and set texture
    ...
    private void visFieldCopyPasteGui()
    {
        GUILayout.Window(1, vf_Rect, delegate (int id1)
        {
            scrollPosition = GUILayout.BeginScrollView(scrollPosition);
            ...
            GUILayout.BeginVertical();
            GUILayout.Space(390); // hack!
                                  //var ta = GUILayout.TextArea("dfdfdf", 1000, textAreaLayoutOptions);

        GUILayout.BeginHorizontal();
            if (GUILayout.Button("Compute", visFieldButtonOptions))
            {
                Debug.Log("Computing..");
                string rawtxt = visFieldInputCanvas.GetComponentInChildren<UnityEngine.UI.InputField>().text;
                string[] args = rawtxt.Split(',');

            // check if invalid
            if (args.Length == 0 || args.Length % 3 != 0)
                {
                    visFieldInputCanvas.GetComponentInChildren<UnityEngine.UI.InputField>().text = "";
                    visFieldInputCanvas.GetComponentInChildren<UnityEngine.UI.InputField>().placeholder.GetComponent<UnityEngine.UI.Text>().text = "Invalid input";

                }
                else
                {
                // get grid
                int nTriplets = args.Length / 3;
                    double[,] xy = new double[nTriplets, 3];
                    for (int i = 0; i < nTriplets; i++)
                    {
                        xy[i, 0] = double.Parse(args[i * 3 + 0]);
                        xy[i, 1] = double.Parse(args[i * 3 + 1]);
                        xy[i, 2] = double.Parse(args[i * 3 + 2]);
                    }
            ...
            {
                expanded = true;
                showVisFieldCopyPasteGui = false;
                visFieldInputCanvas.gameObject.SetActive(false);
            }
            GUILayout.EndHorizontal();
            GUILayout.EndVertical();
            GUILayout.EndScrollView();
        }, "Visual Field DLS Copy-Paste");
